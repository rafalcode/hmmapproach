...
#define EP .0000001 /* the usual epsilon for deciding when two floats are close enough to each other */

void chkrows(float *m) /* checks whether a row exceeds unity. Not quite complete I don't think */
{
    int i, j;
    float su;
    for(i=0;i<DIM1;++i) {
...
void mm_us(float *m1, float *m2, float *mout) /* basic matrix mutliplication: mnemonic is matrixmultiply us? */
{
    int i, j, k;
    float su;
    for(i=0;i<DIM1;++i)
...
void mmltg(float *m, float *tmm, int times) /* multiply the matrix m by itself a number of times, store result in tmm */
{
    int i;
...
{
    char s[DIM2]={'A', 'C', 'G', 'T'};
    /* Here we hard code the examples fro the Durbin Eddy book:
